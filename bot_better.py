...
def processar_dados(texto):
    ...
    for linha in linhas:
        ...
        # Novo item começa com número
        if re.match(r"^\d+$", linha) or re.match(r"^\d+\s", linha):
            # Processa o item anterior se existir
            if item_iniciado and item_atual:
                produto = extrair_dados_produto(item_atual)
                if produto:
...
def extrair_dados_produto(item):
    try:
        # Ignorar cabeçalhos, dados de cliente ou outros dados não relacionados a produtos
        if any(palavra in ' '.join(item) for palavra in ["CNPJ", "Inscrição", "Endereço", "CEP", "Valor total", "Data de Emissão"]):
            return None
        
        # Buscar nome do produto
        nome = ""
        for i, linha in enumerate(item):
            if i > 0 and not re.match(r"^\d+(\s|$)", linha) and not re.match(r"^R\$", linha):
                nome += linha + " "
                if len(nome) > 5:  # Garantir que temos um nome válido
                    break
        nome = nome.strip()
        
        # Buscar quantidade e unidade
        ...
        for linha in item:
            # Procurar por padrões como "3 UN", "1.5 KG", etc.
            qtd_match = re.search(r"(\d+[.,]?\d*)\s*(UN|KG|G)", linha, re.IGNORECASE)
            if qtd_match:
        ...
        valor = 0
        for linha in item:
            # Procurar por padrões como "R$ 35,90"
        ...
        if nome and (estoque_quant > 0 or estoque_peso > 0):
            return {
                "product_name": nome,
        ...
        print(f"Erro ao extrair dados do produto {item}: {e}")
        return None
